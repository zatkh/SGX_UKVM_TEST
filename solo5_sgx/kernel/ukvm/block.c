...
solo5_result_t solo5_sgx_block_write( const void *buf,
        size_t size)
{
    ...
    ukvm_do_hypercall(UKVM_SGX_BLKWRITE, &wr);

    return (wr.ret == 0) ? SOLO5_R_OK : SOLO5_R_EUNSPEC;
}

solo5_result_t solo5_sgx_block_init(void)
{
    struct ukvm_test str;

    ukvm_do_hypercall(UKVM_SGX_BLKINIT,&str);

    return SOLO5_R_OK ;
}

solo5_result_t solo5_sgx_block_seek(void)
{

    ukvm_do_hypercall(UKVM_SGX_BLKSEEK,NULL);

    return SOLO5_R_OK ;
}


solo5_result_t solo5_sgx_block_read( void *buf, size_t size)
{
    ...
    ukvm_do_hypercall(UKVM_SGX_BLKREAD, &rd);
